...
switch mode
    case 0
        ...
        gamma=2.5;  %learning rate
        variance=0.1; 
        test_bias_1=[0,0.001,0.002,0.003];
        test_bias_2=[0.01,0.02,0.03];
    case 1
        bias=max(abs(approximation_aim));
        approximation_aim=approximation_aim+bias;
        gamma=2.5;  %learning rate
        variance=0.1; 
        test_bias_1=[0,0.02,0.023,0.026,0.029];
        test_bias_2=[0.3,0.35,0.4,0.45,0.5];
    case 2
        bias=2*max(abs(approximation_aim));
        approximation_aim=approximation_aim+bias;
        gamma=2.5;  %learning rate
        variance=0.1; 
        test_bias_1=[0,0.02,0.022,0.024,0.026];
        test_bias_2=[0.3,0.34,0.38,0.42];
...
bias2=sum(approximation_aim)/length(approximation_aim);
