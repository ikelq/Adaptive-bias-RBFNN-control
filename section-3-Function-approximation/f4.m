...
x=[reshape(x1,1,numel(x1));reshape(x2,1,numel(x2))];
 approximation_aim= 42.695*(0.1+x1.*(0.05+x1.^4-10*x1.^2.*x2.^2+5*x2.^4));
...
switch mode
    case 0
        ...
        gamma=3.5;  %learning rate
        variance=0.08; 
        test_bias_1=[0,0.001,0.002,0.01];
        test_bias_2=[0.01,0.02,0.1];
    case 1
        bias=max(abs(approximation_aim));
        approximation_aim=approximation_aim+bias;
        gamma=3.5;  %learning rate
        variance=0.08; 
        test_bias_1=[0,0.012,0.014,0.016,0.018,0.02];
        test_bias_2=[0.2,0.24,0.28,0.3,0.32];
    case 2
        bias=2*max(abs(approximation_aim));
        approximation_aim=approximation_aim+bias;
        gamma=3.5;  %learning rate
        variance=0.08; 
        test_bias_1=[0,0.014,0.016,0.017,0.018,0.02];
        test_bias_2=[0.24,0.26,0.28,0.3,0.32];
...
x_t=[reshape(x1_t,1,numel(x1_t));reshape(x2_t,1,numel(x2_t))];
 y_test= 42.695*(0.1+x1_t.*(0.05+x1_t.^4-10*x1_t.^2.*x2_t.^2+5*x2_t.^4))+bias;
%y_test=4*x1_t.*x2_t;
